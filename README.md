...
site/
  ...
  assets/js/main.js     — слайдер, меню, галерея-лайтбокс, табы, анимации, поиск
  assets/js/i18n.js     — переводы AZ / EN / TR
...
## Языки — AZ / EN / TR
Переключатель в топбаре (на мобилке — внизу бургер-меню). Выбор запоминается в localStorage.

Разметка не содержит ключей: `assets/js/i18n.js` — это словарь, где **ключ = точная
азербайджанская строка из HTML**, значение — `{en, tr}`. При переключении JS обходит текстовые
узлы и подменяет совпадения (плюс атрибуты `aria-label`, `placeholder`, `title`).

Поэтому: **изменил азербайджанский текст в index.html — поправь такой же ключ в i18n.js**,
иначе строка просто останется на азербайджанском (не сломается, но не переведётся).
Имена людей, названия компаний и инициалы намеренно не в словаре.

Найти непереведённое можно в консоли: переключиться на EN и поискать в тексте страницы
символы `ə`/`ı`, которых в английском быть не должно.

## Поиск
Кнопка «Axtarış» в меню (на мобилке — иконка). Индекс строится из DOM при каждом открытии,
поэтому автоматически ищет на текущем языке. Индексируются заголовки секций и блоки
`.docrow, .panel-slide, .speaker, .leaf-person, .partner, .edition`.
Диакритика сворачивается (`ə→e`, `ı→i`, `ş→s`…), так что «muzakire» находит «müzakirə».
Результат внутри карусели домотает трек до нужного слайда и подсветит его.
